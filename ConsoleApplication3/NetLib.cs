...
namespace WalaNet
{
    ...
    public class Dneuron //Neuron used in dynamic networks
    {
        //Data structure
        private Expression efunc;//Evaluation function for the neuron
        private int[] pointers;//ID's of other neurons in the network it is pointing to
        private double[] weights;//Weights of respective pointers
        private double bias;//Neuron's bias weight
        private int stepcount = 0;//Neuron's internal step counter
        private int skip = 0;//Step counter limit
        private double threshold = 1;//Threshold for a refractory neuron to consider itself to have "fired"
        private bool refractory;//Is the neuron activity step dependent or independent?
        private bool hasEfunc = false; //Is there an evaluation function??
        ...
        }//Is the neuron active? i.e. is it able to output a value?
        public Expression evalFunction
        {
            get
            {
                return efunc;
        ...
        }//Evaluation function for the neuron
        public int[] Sources
        {
            get
            {
        ...
        }//ID's of other neurons in the network it is pointing to
        public double Bias
        {
            get
            {
        ...
        public double refractoryThreshold
        {
            get
            {
                return threshold;
        ...
        //Utilities
        private int findID(int ID)//returns index in pointers/weights of specified Neuron ID
        {
        ...
        //Constructors
        public Dneuron() { }//Default
        public Dneuron(Expression eFunc)
        {
            efunc = eFunc;
            hasEfunc = true;
        }
        public Dneuron(int[] Pointers, double[] Weights)
        {
            pointers = Pointers;
            weights = Weights;
        }
        public Dneuron(Expression eFunc, int[] Pointers, double[] Weights)
        {
            efunc = eFunc;
            pointers = Pointers;
            weights = Weights;
        ...
        public void addPointer(int NeuronID, double weight)
        {
            pointers = Utils.extendArray(pointers, 1);
            weights = Utils.extendArray(weights, 1);
            pointers[pointers.Length-1] = NeuronID;
            weights[weights.Length - 1] = weight;
        }//Add a pointer
        public void deletePointer(int NeuronID)
        {
            if (pointers.Contains(NeuronID))
            {
                int tmp = findID(NeuronID);
                pointers = Utils.removeArrayElement(pointers, tmp);
                weights = Utils.removeArrayElement(weights, tmp);
            }
            else
            {
                throw new KeyNotFoundException();
        ...
        public void updatePointer(int NeuronID, double newWeight)
        {
            weights[findID(NeuronID)] = newWeight;
        }//Change the weight of a pointer
        public double getWeight(int NeuronID)//Get weight of a pointer
        {
            return weights[findID(NeuronID)];
        }

        //Return an evaluation value. Returns 0 if neuron is inactive. Also steps the neuron.
        //If the neruon has no evaluation function, it returns the sum of all weighted values.
        public double evaluate(double[] NeuronVals)
        {
            if (isActive)
            {
                step();
                ...
                for (int i = 0; i < pointers.Length; i++)
                {
                    retval += NeuronVals[pointers[i]] * weights[i];
                }
                if (hasEfunc)
                {
                    retval = efunc.evaluate(new double[] { retval });
                }
                if (refractory)
                {
                    if (Math.Abs(retval) > threshold)
                    {
                        stepcount = 0;
                    }
                }
        ...
        }//neuron activity controller.

    }

    public class Dnet //Dynamic neural network.
    {
        double[] nStates;//Internal value held by each neuron's previous evaluation.
        int[] inputIDs;
        int[] outputIDs;
        Dneuron[] network;//Neurons in network
        
        //type 0 = classical neural network
        //type 1 = dynamic neural network with classical output (internal values and outputs range from 0 to 1)
        //type 2 = dynamic neural network with flexible evaluation
        public Dnet(int inputs, int outputs, int workers, int type = 1)//'Workers' are Dneurons. These can function as inputs and outputs
        {
            if (workers < 1)
            {
                throw new ArgumentOutOfRangeException("Workers", "must be greater than or equal to 1");
            }
            if (inputs < 1)
            {
            ...
            if (outputs < 1)
            {
                throw new ArgumentOutOfRangeException("Outputs", "must be greater than or equal to 1");
            }
            switch (type)
            {
                case 0:
                    ...
                    break;
                case 1:
                    network = new Dneuron[workers];
                    //TODO figure out how Dneurons should be evaluated
                    Expression e = new Expression("x");//Standard sigmoid function is 1/(1+e^(-x))
                    for (int i = 0; i < workers; i++)
                    {
                        network[i] = new Dneuron();//Network is completely empty and unconnected. Essentially just material to work with here.
                    }
                    inputIDs = new int[inputs];
                    //INPR: Fill this list out (at random??) with ID's
                    break;
                case 2:
                    //TODO: This
                    break;
